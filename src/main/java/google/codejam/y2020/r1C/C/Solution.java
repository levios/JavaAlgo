package google.codejam.y2020.r1C.C;
...
public class Solution {
    ...
    private static boolean noGap(Map<Long, Integer> slices) {
        Set<Map.Entry<Long, Integer>> x = slices.entrySet().stream().filter(e -> e.getValue() == 2).collect(Collectors.toSet());
        for (Map.Entry<Long, Integer> entry: x) {
            if (slices.keySet().stream().max(Comparator.naturalOrder()).get() > entry.getKey()) {
                return false;
            }
        }
        return true;
